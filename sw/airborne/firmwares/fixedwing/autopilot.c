...
#include "firmwares/fixedwing/navigation.h"
...
// in flight time
uint32_t autopilot_flight_time;
...
// FIXME - guidance_v/h mode as in rotorcraft
...
#define AUTOPILOT_IN_FLIGHT_MIN_THRUST 0
#endif
 ...
 */
//QUICKFIX - move to airframe config
#define USE_THROTTLE_FOR_MOTOR_ARMING TRUE
...
void autopilot_periodic(void) {
   ...
   */
   // TODO: detect ground only during landing phase?
...
INFO("Using FAILSAFE_GROUND_DETECT: KILL")
  ...
  if (autopilot_mode == AP_MODE_KILL) {
    ...
    SetAutopilotCommands(stabilization_cmd, autopilot_in_flight, autopilot_motors_on);
  }
  ...
  if (new_autopilot_mode != autopilot_mode) {
    ...
    switch (new_autopilot_mode) {
      case AP_MODE_FAILSAFE:
        ...
        // TODO stabilization_attitude_set_failsafe_setpoint();// Rename !
    ...
    switch (new_autopilot_mode) {
      case AP_MODE_FAILSAFE:
        ...
        guidance_v_zd_sp = SPEED_BFP_OF_REAL(0.5); // FAILSAFE_V_ZD instead? 
        // setpoints are already set in "horizontal mode"
        // i.e. stabilization_attitude_set_failsafe_setpoint();
        break;
#endif
      case AP_MODE_KILL:
        autopilot_set_motors_on(FALSE);
        stabilization_cmd[COMMAND_THROTTLE] = 0;
...
void autopilot_check_in_flight(bool_t motors_on) {
  if (autopilot_in_flight) {
    if (autopilot_in_flight_counter > 0) {
      ...
      // TODO forward velocity instead of vertical, use pitot tube?
      if ((stabilization_cmd[COMMAND_THROTTLE] <= AUTOPILOT_IN_FLIGHT_MIN_THRUST) &&
          ...
          (abs(stateGetAccelNed_f()->z) < AUTOPILOT_IN_FLIGHT_MIN_ACCEL))
      {
        ...
        autopilot_in_flight_counter = AUTOPILOT_IN_FLIGHT_TIME_THRESHOLD;
      }
    }
  }
  else { /* currently not in flight */
    if (autopilot_in_flight_counter < AUTOPILOT_IN_FLIGHT_TIME_THRESHOLD &&
        motors_on)
    {
