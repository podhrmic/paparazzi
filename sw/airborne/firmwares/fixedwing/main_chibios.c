 ...
 * Identical to rotorcraft, that is the aim of refactoring
 *
...
#include "firmwares/fixedwing/telemetry.h"
...
// TODO
// probably rename to stabilization & navigation
...
void on_gyro_event( void )
{
  ...
  if (ahrs.status == AHRS_UNINIT)
  {
   // FIXME ahrs_aligner_run();
    // FIXME if (ahrs_aligner.status == AHRS_ALIGNER_LOCKED)
    //  ahrs_align();
  }
  else {
    ahrs_propagate();
...
static __attribute__((noreturn)) msg_t thd_radio_control(void *arg)
{
  ...
  while (TRUE)
  {
    ...
    // TODO radio_control_periodic_task();
    chThdSleepUntil(time);
  }
}
...
static __attribute__((noreturn)) msg_t thd_radio_event(void *arg)
{
  ...
  // FIXME EventListener elRadioEvt;
  // TODO chEvtRegister(&eventPpmFrame, &elRadioEvt, EVT_PPM_FRAME);
  // FIXME flagsmask_t rc_flags;

  while (TRUE)
  {
    // TODO chEvtWaitOne(EVENT_MASK(EVT_PPM_FRAME));
    // rc_flags = chEvtGetAndClearFlags(&elRadioEvt);
    // if (rc_flags & EVT_PPM_FRAME)
    {
      if (autopilot_rc)
      {
        // TODO RadioControlEvent(autopilot_on_rc_frame);
        // TODO chEvtBroadcastFlags(&eventRadioData, EVT_RADIO_DATA);
      }
    }
